...
export const createCourse = async (req, res) => {
  try {
    ...
    console.log("createCourses error:", err);
    return res.status(500).json({
      success: false,
      error: "Server Error",
    });
...
export const rateCourse = async (req, res) => {
  try {
    ...
    course.avgRating = avgRating; //if a particular course has multi user rating then it will computer
...
export const getMyRating = async (req, res) => {
  try {
    ...
    const my =
      (course.rating || []).find((r) => String(r.userId) === String(userId)) ||
      null;
    return res.json({
      success: true,
